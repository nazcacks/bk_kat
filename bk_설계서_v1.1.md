...
| 신규 메뉴 산출물 | 메뉴구조도 V1.0(3채널 4단: 운영콘솔 OP-00~11 / 업무화면 TN-00~15 / 공통 CO-01~10) + 4단 세부 메뉴구조 반영 | 0.5, 26장 |
...
| 화면설계 | `설계/화면설계/메뉴구조도_V1.0.HTML`, `설계/화면설계/메뉴구조.html` (v1.1 신규) | 3채널 4단 메뉴구조·SA 182 화면 매핑 기준 (26장) |
...
| W6 | **운영 콘솔 공통 표준** — 접근모드 4종 배너 고정, 역할 6종(BK_MANAGER/BK_PREPARER/BK_REVIEWER/SEC_ADMIN/AUDITOR/BILLING) 접근 매트릭스, 오류코드 10종 표준, 권한 없는 메뉴 미노출 원칙 | 운영콘솔_화면_v1.0.html §2·§5 | 26.2~26.4 | 반영 |
| W7 | **메뉴구조 3채널 4단 확정** — 1뎁스 채널(운영콘솔 OP-00~11 / 업무화면 TN-00~15 / 공통·기술·설계 추적 CO-01~10), 2뎁스 업무 영역, 3뎁스 진입 메뉴, 4뎁스 화면·탭·기능. SA 182개 화면을 TN 메뉴에 전수 배치 | 메뉴구조도_V1.0.HTML | 26.1 | 반영 |
...
- (v1.1 병합) `User.loginId`는 전역 유일. 사용자 그룹별 소속 필드는 조건부 필수: `TENANT` 사용자는 `homeTenantId`, `OPERATOR` 사용자는 `operatorOrgId` 필수.
...
- (v1.1 병합) 관리회사 운영자 Role 체계(운영 콘솔): **운영 총괄(슈퍼관리자)**=전 기능+정책, **운영 관리자**=계약·온보딩·요금, **고객 지원**=지원세션 범위, **기준정보 운영**=표준 카탈로그, **시스템 관리자**=인프라·배치·보안. 기장 담당 Role(BK_MANAGER/BK_PREPARER/BK_REVIEWER)은 4.5, 운영 콘솔 역할 6종 접근 매트릭스는 26.3을 따른다.

**(v1.1 병합) 메뉴·권한 평가 모델** — 화면설계 확정분을 데이터 모델·평가 규칙으로 정의한다.

- 메뉴 유형: `GROUP`(그룹) / `MENU`(진입 메뉴) / `TAB`(탭) / `ACTION`(버튼·행위).
- 권한 엔티티 분리: `RoleMenuPermission`(메뉴 노출·진입) + `MenuActionGrant`(버튼 단위 행위: 조회/등록/수정/승인/배포/회수/전송). 12.1의 `MenuPermissionLink`는 이 2개 엔티티로 세분한다(명칭 통일).
...
운영 콘솔의 관리회사·조직 화면(OP-02, 26장)이 사용하는 운영조직 도메인을 정의한다.

**(1) 엔티티**

| 엔티티 | 핵심 필드 | 규칙 |
|---|---|---|
| `OperatorOrg` | 관리회사명, 사업자번호, **세무대리 등록번호**, **홈택스 세무대리인 식별자**, 기본 언어/시간대, 상태 | 관리회사(운영조직) 최상위 |
| `OperatorOrgUnit` | 상위 조직 참조(계층), 조직코드, 조직명 | 관리회사 내 조직코드 유일, **순환참조 금지** |
| `OperatorTenantLink`(=`TenantAssignment`) | operatorUserId/orgUnitId, tenantId, 역할, 유효기간 | 동일 회사에 대한 **활성 담당 관계는 1건**으로 제한 |

**(2) 운영 콘솔 IP 허용목록(`ConsoleIpAllowlist`)**
...
- 담당 배정 범위 원칙: 운영자·기장 담당은 배정(`TenantAssignment`/`BookkeepingAssignment`) 범위 내 회사만 조회·검색·작업 가능(미배정 회사 미노출, 최소 권한). 배정 변경은 활동 이력에 기록한다.
...
| 메뉴 마스터(v4.1, BF-12) | `Menu`(type: GROUP/MENU/TAB/ACTION), `MenuVersion`(DRAFT/PUBLISHED 발행·롤백), `RoleMenuPermission`(메뉴 노출·진입), `MenuActionGrant`(버튼 단위 행위 — v1.1: 구 `MenuPermissionLink`를 2개 엔티티로 세분, 4.3) |
| 운영조직(v1.1, 5.5) | `OperatorOrg`(세무대리 등록번호·홈택스 식별자 포함), `OperatorOrgUnit`(계층·순환참조 금지), `OperatorTenantLink`/`TenantAssignment`(활성 담당 1건), `ConsoleIpAllowlist`(CIDR), `OperatorActivityLog`(append-only) |
...
| Scope | 예: `tenant:journal:write`, `tenant:journal:approve`, `tenant:journal:post`, `tenant:masterdata:write`, `tenant:closing:approve`, `tenant:closing:reopen`, `tenant:withholding:submit`, `tenant:bookkeeping:acknowledge`, `tenant:filing:approve`, `operator:tenant:support`, `operator:masterdata:write`, `operator:bookkeeping:prepare`, `operator:bookkeeping:close`, `operator:breakglass:approve`, `security:pii:decrypt`. 위험 scope(마감해제·신고 전송·평문 조회)는 Step-up 인증을 요구한다. (v1.1) scope 명칭은 본 표를 단일 기준으로 하며, 화면설계 산출물의 상이 표기는 본 표 기준으로 통일한다. |
| 멱등성 | 생성·전송·반영 API는 `Idempotency-Key` 헤더를 받으며, 동일 키+동일 요청해시는 동일 응답을 반환한다. 동일 키+다른 요청해시는 `409 IDEMPOTENCY_CONFLICT` 처리한다. |
| 오류코드 | 공통 오류: `VALIDATION_FAILED`, `UNAUTHORIZED`, `FORBIDDEN_SCOPE`, `TENANT_ACCESS_DENIED`, `SOD_VIOLATION`, `CLOSING_LOCKED`, `PERIOD_LOCKED`(신고 완료 기간 마감해제 불가), `CHECKLIST_NOT_PASSED`(마감 체크리스트 미통과), `EVIDENCE_QUARANTINED`(증빙 검사 격리), `IDEMPOTENCY_CONFLICT`, `EXTERNAL_INTEGRATION_FAILED`, `RATE_LIMITED`, `LOCK_NOT_ACQUIRED`, `NOT_PRIMARY_BOOKKEEPER`(비주기장 마감/결산/신고 시도), `BOOKKEEPING_NOT_CONTRACTED`(기장 계약 없는 회사 기장 API), `ASSIGNMENT_REQUIRED`(담당 배정 없는 기장 접근). (v1.1 병합) 전표 도메인: `AUX_REQUIRED`(필수 보조부/관리항목 누락), `DIMENSION_REQUIRED`(필수 차원 누락), `POSTED_IMMUTABLE`(기표 전표 직접 수정 시도 — 역분개 안내), `QUICK_ENTRY_INVALID_ACCOUNT`(간편입력 부적격 계정), `NUMBERING_CONFLICT`(채번 충돌). 운영 콘솔 오류코드 10종은 26.4를 따른다. |
...
**(v1.1 병합) 신규 온보딩 절차·게이트** — ① 관리회사 준비(조직·담당 배정) → ② 표준 배포(계정/세율/역할/서식) → ③ 테넌트 생성(격리 티어) → ④ 구독 개설 → ⑤ 기장 모드 설정 → ⑥ 사용자·인증 정책 → ⑦ 역할·결재선·배정 → ⑧ 표준 채택·차원(`DimensionConfig`)·알림 설정 → ⑨ 기준정보·개시잔액 적재 → ⑩ 업무 개시. **개시잔액 차대변 불균형 시 `ACTIVE` 전환을 차단**한다(균형 검증 게이트). 미결 채권채무·어음·고정자산 이관은 `MigrationBatch`/`OpeningBalance` 단위로 검증·적재한다(부록 C 22장 준용).
...
`설계/화면설계/메뉴구조도_V1.0.HTML`(3채널 4단 메뉴구조도, 2026-06-30)·`설계/화면설계/메뉴구조.html`(4단 세부 메뉴구조 그리드, SA 182개 화면 전수 수록)·`설계/화면설계/운영콘솔_화면_v1.0.html`(운영 콘솔 화면설계 v1.0, 2026-07-01)을 본 문서의 메뉴·화면 권위 기준으로 정의한다. `참고자료/메뉴구조도_v0.1.html`(구 v4.0 체계)은 V1.0으로 대체된 참고자료다.

### 26.1 3채널 4단 메뉴구조 (메뉴구조도 V1.0)

구조 원칙: **1뎁스 = 채널**(운영콘솔/업무화면/공통), **2뎁스 = 업무 영역**(메뉴 코드), **3뎁스 = 진입 메뉴**, **4뎁스 = 화면·탭·실행 기능**.

**(1) 운영콘솔 채널 (OP-00~OP-11)** — 관리회사·운영자·기장 담당
...
| OP-02 | 관리회사·조직 | 관리회사·조직(BF-01: OperatorOrg 트리·담당 테넌트 배정·IP 허용목록), 운영 조직 권한·활동 이력 | 5.5 |
...
| OP-05 | 공통 표준 카탈로그 | 표준 카탈로그(BF-08: 8종 표준·버전·배포 3모드·diff·롤백), 표준 배포 관리(채택 현황·회귀 검증) | 6장 |
| OP-06 | 사용자·인증·권한 | 사용자 관리(BF-04), 인증 정책(BF-05), 역할·가드레일·결재선(BF-07), 메뉴·권한 관리(BF-12) | 3장, 4장 |
...
**(2) 업무화면 채널 (TN-00~TN-15)** — 이용회사·기장 업무 메뉴

| 메뉴 코드 | 2뎁스 메뉴 | 주요 3~4뎁스 (SA 화면군 배치) | 연계 |
|---|---|---|---|
| TN-00 | 홈·현황 | 회사 대시보드(신고 기한·미결 전표·마감 상태·질의/확인 대기·이상 알림) | 18장 |
| TN-01 | 회사 설정·기초 | 기장 방식(BF-03), 차원/관리항목(BF-09: 6차원·DimensionConfig·소수점), 알림 설정(BF-11) | 21장, 9장, 18장 |
...
**(3) 공통·기술·설계 추적 채널 (CO-01~CO-10)** — CO-01 로그인·내 계정(3장), CO-02 알림·커뮤니케이션(18장), CO-03 API·서비스 계약(13장), CO-04 검증·오류 처리(15장), CO-05 데이터 모델·상태값(12장), CO-06 배치·스케줄(14장), CO-07 보안·개인정보(17·20장), CO-08 시스템 운영·인프라(19장), CO-09 산출물·보고서(11.13, 24장), CO-10 설계서 부록 추적(부록).

**(4) SA 182개 화면 배치 합계** — SA-BAS 14(TN-02), SA-OPN 19(TN-03·08), SA-JNL 4(TN-04), SA-ATX 10(TN-06), SA-LDG 29(TN-07·05), SA-CLS 21(TN-08), SA-VAT 50(TN-09), SA-FA 7(TN-10), SA-FND 14(TN-11·12), SA-DEP 3(TN-11), SA-BIL 4(TN-11), SA-DAT 7(TN-15) = **182**.

**(5) 메뉴 운영 규칙** — ① 권한 없는 메뉴 미노출(4.3), ② 기장 모드 연동(모드별 메뉴 가변·배지, 21.8), ③ 마감 상태 `CLOSING_LOCKED` 시 쓰기 메뉴 제한, ④ 상태 불변성(`POSTED`/`CLOSED`/`LOCKED` 직접 수정 금지), ⑤ 기능 플래그(구독·업종·Phase·회사 설정 연동 노출 또는 업그레이드 안내 — 외화·관계사·Cost-plus·예산·그룹보고·제조원가/재고·XBRL·다중 회계기준), ⑥ 메뉴 접근·행위 감사 추적.

### 26.2 운영 콘솔 공통 표준 (운영콘솔_화면_v1.0)

- **셸 레이아웃**: 상단 GNB(운영자 컨텍스트·이용회사 선택·운영 알림·사용자) + 좌측 LNB(OP 메뉴, `RoleMenuPermission` 렌더링·`MenuActionGrant` 버튼 노출) + 콘텐츠. **테넌트 컨텍스트 진입 시 전 화면 상단에 접근모드 배너 고정**(`VIEW`/`SUPPORT·잔여시간·사유`/`BOOKKEEPING·회사·역할`/`BREAK_GLASS·녹화 중 ●`).
...
역할 6종: `BK_MANAGER`(운영 관리자) / `BK_PREPARER`(기장 작성) / `BK_REVIEWER`(기장 검토) / `SEC_ADMIN`(보안 관리자) / `AUDITOR`(감사자) / `BILLING`(청구 담당). (○ 전체, △ 범위 제한, - 미노출)

| 화면 | BK_MANAGER | BK_PREPARER | BK_REVIEWER | SEC_ADMIN | AUDITOR | BILLING |
|---|---|---|---|---|---|---|
| OP-00 운영 진입 | ○ | ○ 담당 | ○ 담당 | △ | △ 조회 | △ |
| OP-01 운영 현황 | ○ | △ 담당 | △ 담당 | ○ | △ 조회 | △ 사용량 |
| OP-02 관리회사·조직 | ○ | - | - | △ IP | △ 활동이력 | - |
| OP-03 테넌트 | ○ | △ 담당 조회 | △ 담당 조회 | △ 격리·스냅샷 | △ 조회 | △ 상태 |
| OP-04 구독·요금 | ○ | - | - | - | △ 조회 | ○ |
| OP-05 표준 카탈로그 | ○ | △ 조회 | △ 조회 | △ 조회 | △ 조회 | - |
| OP-06 사용자·인증·권한 | ○ | - | - | △ 인증정책 | △ 조회 | - |
| OP-07 접근 거버넌스 | ○ 승인 | △ 요청 | △ 요청 | ○ 녹화재생 | ○ 검토 | - |
| OP-08 기장 워크벤치 | ○ 관리 | ○ 담당 | ○ 검토 | - | △ 검토 | - |
| OP-09 배치·연계 | ○ | △ 담당 | - | △ 볼트·인증서 | △ 조회 | - |
| OP-10 보안·감사 | △ 조회 | - | - | ○ | ○ 감사 | - |
| OP-11 AI·규칙 | ○ | △ 조회 | △ 조회 | △ 가드레일 | △ 조회 | - |

실제 노출은 `RoleMenuPermission`·`MenuActionGrant`·담당 테넌트 배정 범위·기장 모드·구독/기능 플래그를 결합해 결정한다.

### 26.4 운영 콘솔 오류 코드 표준 (10종)
...
| `STANDARD_CONFLICT` | 회사 확장 항목과 표준이 충돌합니다. diff에서 충돌을 해소하세요. | OP-05 |
...
**(1) BF → OP 재배치 매핑**: BF-00→OP-00A, BF-01→OP-02, BF-02→OP-03, BF-03→OP-08A(운영측)·TN-01(테넌트측), BF-04→OP-06A, BF-05→OP-06B, BF-06→OP-07, BF-07→OP-06C, BF-08→OP-05, BF-09→**TN-01(테넌트측 확정)**, BF-10→OP-04, BF-11→**TN-01(테넌트측 확정)**, BF-12→OP-06D.
...
**(3) 정정 사항(v1.1)**: ① 기장 워크벤치 `OP-12` → **`OP-08`**, ② 구 "OP-02 테넌트 상세 기장 설정 탭" → 신 체계 **OP-08A 독립 화면**(OP-02는 관리회사·조직, OP-03이 테넌트), ③ SA-BAS-08~13 ID-화면명은 25장 절 번호 기준(08 현장등록·09 프로젝트등록·10 업무용승용차·11 외주처등록·12 거래처DM인쇄·13 거래처등코드변환)으로 확정 — `메뉴구조도_V1.0.HTML` 트리의 상이 표기(11 차량등록·12/13 외주처/외주품목)는 차기 개정 시 정정 대상.
