...
| 신규 메뉴 산출물 | 메뉴구조도 V1.0의 3채널에 플랫폼콘솔 PF-00~06을 추가한 **4채널 4단 구조** 반영 | 0.5, 26장 |
...
| 시스템관리자 화면 통합(07-13) | `설계/페이즈_화면/시스템관리자_화면_v1.0.HTML`의 사용자 목록/상세·보안/세션, 사용자그룹/구성원, 그룹별 메뉴권한(`INHERIT/ALLOW/DENY`), 메뉴 마스터 필드·버전·영향분석을 수용. 설계서의 3계층·플랫폼 표준 단일소유 원칙을 우선하여 메뉴 원본 CUD는 `PF-04M`으로 승격하고 OP/TN은 범위별 사용자·그룹·권한만 관리하도록 정합화 | 4.3, 5.7, 12~15장, 22장, 26.1·26.4·26.6 |
...
| 화면설계 | `설계/화면설계/메뉴구조도_V1.0.HTML`, `설계/화면설계/메뉴구조.html` (v1.1 신규) | 원본 3채널 + 본문 PF 플랫폼콘솔 확장, SA 182 화면 매핑 기준 (26장) |
...
| W5 | **운영 콘솔 화면 체계 확정** — OP-00~OP-11 12개 상위 메뉴(+하위 OP-00A/B, 01A/B, 06A~D, 07A/B, 08A/B), BF-00~12 → OP 재배치 매핑. **기장 워크벤치는 `OP-08`** (기존 본문 `OP-12` 표기 정정) | 운영콘솔_화면_v1.0.html | 26장, 21.5 | 반영 |
| W6 | **운영 콘솔 공통 표준** — 접근모드 배너, 플랫폼/관리회사 Role 분리, 3계층 오류코드, 권한 없는 메뉴 미노출 | 운영콘솔_화면_v1.0.html §2·§5 + 3계층 보강 | 26.2~26.4 | 보강 |
| W7 | **메뉴구조 4채널 4단 확정** — 플랫폼콘솔 PF / 관리회사 운영콘솔 OP / 테넌트 업무화면 TN / 공통 CO. SA 182개 화면은 TN 메뉴에 유지 | 메뉴구조도_V1.0.HTML + 3계층 보강 | 26.1 | 보강 |
...
| W9 | **메뉴 코드와 화면 ID 표기 구분** — 2뎁스 메뉴 코드(TN-xx)와 구 화면 ID(TN-xx)의 동명 충돌은 "메뉴 코드"와 "화면 ID" 속성으로 구분 표기하고, 화면 ID는 26장 매핑표를 권위 기준으로 한다 | 메뉴구조도 3종 비교 | 26.5 | 반영 |
...
|---|---|---|---|
| 메뉴목록·메뉴 마스터 | `PF-04M` | PLATFORM | 전체 PF/OP/TN/CO 메뉴 원본·다국어·라우트·버전·발행·롤백. 화면설계 `OP-06D`의 편집 UX를 이관 |
| 발행 메뉴 조회·영향 미리보기 | `OP-06D` | OPERATOR_ORG | 현재 PUBLISHED 메뉴 목록과 Role/그룹 영향 읽기전용. 메뉴 원본 CUD·발행 불가 |
| 플랫폼 사용자/그룹/그룹메뉴권한 | `PF-01U` / `PF-01G` / `PF-01P` | PLATFORM_ORG | PlatformUser·PlatformMembership 범위. PF 메뉴권한만 배정 |
| 관리회사 사용자/그룹/그룹메뉴권한 | `OP-06A` / `OP-06G` / `OP-06P` | OPERATOR_ORG | 현재 operatorOrgId 소속·수임범위. 타 관리회사 사용자·그룹 금지 |
| 이용회사 사용자/그룹/그룹메뉴권한 | `TN-01U` / `TN-01G` / `TN-01P` | TENANT | 현재 tenantId 소속. TN/CO 메뉴와 자사 데이터범위만 배정 |

- 화면설계의 사용자그룹 화면에 중복 표기된 `OP-06A`는 본 설계서에서 `OP-06G`로, 독립 그룹별 메뉴권한은 `OP-06P`로 확정한다.
...
`PF-04M`은 메뉴 트리만 편집하는 방식이 아니라 **메뉴목록 그리드를 기준 화면**으로 한다. 트리는 경로 탐색·필터 보조수단이며 조회·정렬·대량검증·영향분석·엑셀 내보내기는 목록을 기준으로 동작한다.
...
| 화면 | 관리대상 | 필수 통제 |
|---|---|---|
| PF-01U | PlatformUser | PLATFORM_USER_ADMIN, 활성 PlatformMembership, 고위험 조치 Step-up·승인, 자기 Role 승격 금지 |
| OP-06A | 현재 OperatorOrg의 OperatorUser 및 명시적으로 위임된 TenantUser | ORG_ADMIN/ORG_SECURITY_ADMIN, operatorOrgId 고정, 수임·위임 없는 TenantUser 변경 금지 |
| TN-01U | 현재 TenantUser | TENANT_ADMIN, tenantId 고정, 상위 플랫폼/관리회사 계정 변경 금지 |
...
`PF-01P`/`OP-06P`/`TN-01P`는 사용자그룹을 선택한 뒤 유효한 PUBLISHED 메뉴를 목록으로 표시한다.
...
관리회사 운영 콘솔의 현재 조직·소속 화면(OP-02)과 플랫폼 콘솔의 관리회사 화면(PF-01)이 공통으로 사용하는 운영조직 도메인을 정의한다.
...
| `UserGroup` | scopeType=`PLATFORM_ORG`, scopeId, groupCode, nameKo/En, groupType, parentGroupId, defaultRoleId, status, validFrom/To | PF-01G의 플랫폼 사용자그룹. 로그인 분류 `userGroup=PLATFORM`과 별도 엔티티이며 동일 scope 안에서 코드 유일 |
...
8. **사용자그룹·메뉴권한**: PF-01G에서 조직·보안 목적 그룹과 구성원을 별도 저장하고 PF-01P에서 발행된 PF 메뉴에 그룹별 정책을 배정한다. 그룹 ALLOW는 메뉴 후보를 열 뿐 Role의 VIEW/ENTER/ACTION을 생성하지 않으며, 어느 소속 그룹에서든 DENY이면 최종 미노출·진입 차단한다.
9. **화면·감사 정합**: PF-01U의 사용자 상세·보안/세션, PF-01G의 그룹·구성원, PF-01P의 권한 미리보기는 4.3의 공통 필드·평가순서를 사용한다. 초대·인증초기화·세션종료·그룹/구성원 CUD·권한 변경은 사유와 전후값을 `PlatformUserActivityLog`에 기록한다.
...
| 메뉴 목록 키 | 메뉴 코드는 `(channel, menu_code, menu_version_id)`, 정렬은 `(menu_version_id, parent_menu_id, sort_order)` 기준 유일성을 검증한다. 활성 진입 메뉴의 route·screenId는 버전 내 중복을 금지하고 GROUP을 제외한 메뉴는 한국어/영어명을 필수로 한다. |
...
시스템관리자 사용자·메뉴 권한은 `ScopedUserLifecycleService`(PF/OP/TN 사용자 조회·초대·상태·인증·세션), `UserGroupService`(scope별 그룹·구성원·유효기간·이력), `GroupMenuPermissionService`(INHERIT/ALLOW/DENY 저장·상위 DENY 전파), `MenuCatalogService`(목록형 메뉴·다국어·route·계층 검증), `MenuVersionService`(검토·승인·발행·정지·롤백·영향분석), `PermissionEvaluationService`(그룹→Role→Action→data scope 최종평가·`/api/me/menus`), `PermissionSimulationService`(사용자/그룹 기준 미리보기·판정근거)를 별도 애플리케이션 서비스로 둔다. MenuCatalog/MenuVersion 쓰기는 PF-04M에만 열고 OP/TN 서비스는 자기 scope의 사용자·그룹·그룹정책만 쓴다.
...
| `GET·POST` | `/api/platform/user-groups` · `/api/platform/user-groups/{id}/members` | PF-01G 플랫폼 사용자그룹·구성원 목록/등록; 그룹과 구성원은 별도 트랜잭션 |
| `GET·PUT` | `/api/platform/user-groups/{id}/menu-permissions` | PF-01P 발행 PF 메뉴의 그룹별 INHERIT/ALLOW/DENY 일괄 저장(낙관적 잠금) |
| `POST` | `/api/platform/user-groups/{id}/menu-permissions/simulate` | 사용자·그룹·시점 기준 최종 메뉴/차단근거 미리보기 |
| `GET·POST` | `/api/platform/menu-versions` · `/api/platform/menu-versions/{versionId}/menus` | PF-04M 메뉴 버전과 목록형 메뉴 원본·다국어·route 등록/조회 |
...
| `GET·POST` | `/api/operator/users` | OP-06A 현재 operatorOrgId 사용자 목록·초대·상태 관리 |
| `POST` | `/api/operator/users/{id}/reset-auth` · `/sessions/revoke` | 관리회사 사용자 MFA 초기화·활성 세션 종료(Step-up·사유) |
| `GET·POST` | `/api/operator/user-groups` · `/api/operator/user-groups/{id}/members` | OP-06G 현재 관리회사 사용자그룹·구성원 관리 |
| `GET·PUT` | `/api/operator/user-groups/{id}/menu-permissions` | OP-06P 발행 OP 메뉴에 대한 그룹별 정책 관리 |
| `POST` | `/api/operator/user-groups/{id}/menu-permissions/simulate` | OP 그룹·사용자의 최종 메뉴권한 미리보기 |
| `GET` | `/api/operator/menus` · `/api/operator/menu-versions/current` | OP-06D PUBLISHED OP 메뉴목록·버전·영향 읽기전용 조회 |
...
| `GET·POST` | `/api/tenant/users` | TN-01U 현재 tenantId 사용자 목록·초대·상태 관리 |
| `POST` | `/api/tenant/users/{id}/reset-auth` · `/sessions/revoke` | 테넌트 사용자 MFA 초기화·활성 세션 종료(Step-up·사유) |
| `GET·POST` | `/api/tenant/user-groups` · `/api/tenant/user-groups/{id}/members` | TN-01G 테넌트 사용자그룹·구성원 관리 |
| `GET·PUT` | `/api/tenant/user-groups/{id}/menu-permissions` | TN-01P 발행 TN 메뉴에 대한 그룹별 정책 관리 |
...
| 화면/메뉴 | 영향 규칙 |
|---|---|
| 운영 콘솔 테넌트 상세(OP-03) | 기장 설정은 별도 화면 **OP-08A(기장 모드 설정, BF-03)** 로 독립. 테넌트 상세에서는 현재 모드·주기장 요약만 표시 |
| 기장 워크벤치(OP-08B) | 담당 회사 보드·BOOKKEEPING 컨텍스트 진입·보고패키지 조립·Open Items·인계(21.5) |
...
| 단계 | 범위 |
|---|---|
| 1단계 | **플랫폼 사용자·내부조직·Membership·사용자그룹·Role/SOD + 3계층 사용자/그룹/그룹별 메뉴권한 + PF-04M 목록형 메뉴 마스터·MenuVersion + 관리회사·수임관계·조직 RLS + 플랫폼 콘솔**, 테넌트 멀티테넌시, 인증/2FA, 권한위임, 관리자접근 로그, 플랫폼 전체 업무 표준 원본·버전·배포, 전표 코어·차원 엔진 기초 |
...
1차 개발(2606~2611)·2차·3차의 페이즈별 개발 순서를 확정한다. 메뉴·화면 ID는 25장(SA-*)·0.2(AUX/JV) 인벤토리와 대응한다.
...
### 26.1 4채널 4단 메뉴구조 (3계층 보강)

구조 원칙: **1뎁스 = 채널**(플랫폼콘솔/관리회사 운영콘솔/테넌트 업무화면/공통), **2뎁스 = 업무 영역**, **3뎁스 = 진입 메뉴**, **4뎁스 = 화면·탭·실행 기능**. 기존 V1.0의 운영콘솔/업무화면/공통 3채널은 유지하고 상위 플랫폼콘솔 채널을 추가한다.
...
| PF-01 | 플랫폼 조직·사용자·관리회사 | PlatformOrg/Unit·PlatformUser·Membership·DutyAssignment·SSO/SCIM, PF-01U 사용자·PF-01G 사용자그룹/구성원·PF-01P 그룹별 PF 메뉴권한, 관리회사 목록·등록·심사·활성·정지·종료·초기 관리자 | 4.3, 5.5.3, 5.7 |
| PF-02 | 플랫폼 계약·쿼터 | 관리회사 구독·요금제·쿼터·통합 청구·독촉 | 7.2 |
| PF-03 | 수임관계·이관 | 관리회사-테넌트 관계, 중복 검증, 이관 승인·컷오버·롤백 | 5.5.4, 21.9 |
| PF-04 | 전역 정책·전체 업무 표준 | 인증 하한·전역 Role, 회계·세무·서식·코드·검증 등 전체 업무 표준 원본·버전, **PF-04M 목록형 메뉴 마스터/MenuVersion**, SOD 발행·폐기·롤백·채택 현황 | 4장, 6장 |
...
| OP-06 | 사용자·인증·권한 | OP-06A 사용자관리, OP-06G 사용자그룹/구성원, OP-06P 그룹별 메뉴권한, OP-06B 인증정책, OP-06C 역할·가드레일·결재선, OP-06D PUBLISHED 메뉴목록·영향 읽기전용. 메뉴 원본 CUD/발행은 PF-04M | 3장, 4.3, 26.6 |
...
| TN-01 | 회사 설정·기초 | TN-01U 사용자관리·TN-01G 사용자그룹/구성원·TN-01P 그룹별 TN 메뉴권한, 기장 방식(BF-03), 차원/관리항목(BF-09: 6차원·DimensionConfig·소수점), 알림 설정(BF-11) | 4.3, 21장, 9장, 18장 |
...
**(4) 공통·기술·설계 추적 채널 (CO-01~CO-10)** — CO-01 로그인·내 계정(3장), CO-02 알림·커뮤니케이션(18장), CO-03 API·서비스 계약(13장), CO-04 검증·오류 처리(15장), CO-05 데이터 모델·상태값(12장), CO-06 배치·스케줄(14장), CO-07 보안·개인정보(17·20장), CO-08 시스템 운영·인프라(19장), CO-09 산출물·보고서(11.13, 24장), CO-10 설계서 부록 추적(부록).
...
| `USER_LOGIN_ID_DUPLICATED` | 이미 사용 중인 로그인 ID입니다. 다른 ID를 입력하세요. | PF-01U/OP-06A/TN-01U |
| `USER_SCOPE_MISMATCH` | 사용자의 소속과 현재 관리 범위가 일치하지 않습니다. | 4.3.3 |
| `USER_GROUP_CODE_DUPLICATED` | 현재 범위에서 이미 사용 중인 사용자그룹 코드입니다. | PF-01G/OP-06G/TN-01G |
...
| `USER_GROUP_MENU_CONFLICT` | 같은 메뉴에 상충하는 그룹 정책이 요청되었습니다. DENY 우선 규칙과 저장값을 확인하세요. | PF-01P/OP-06P/TN-01P |
| `MENU_VERSION_NOT_PUBLISHED` | 발행된 메뉴 버전만 권한평가와 사용자 메뉴에 사용할 수 있습니다. | PF-04M |
| `MENU_CODE_DUPLICATED` | 같은 채널·버전에 이미 존재하는 메뉴 코드입니다. | PF-04M |
| `MENU_ROUTE_DUPLICATED` | 같은 버전에 이미 사용 중인 화면 경로입니다. | PF-04M |
| `MENU_PARENT_INVALID` | 상위 메뉴의 채널·레벨·유형이 올바르지 않습니다. | PF-04M |
| `MENU_CYCLE_DETECTED` | 메뉴 상하위 관계에 순환이 발생합니다. | PF-04M |
| `MENU_I18N_REQUIRED` | 한국어·영어 메뉴명을 모두 입력하세요. | PF-04M |
| `MENU_IN_USE` | Role·그룹·라우트에서 사용 중인 메뉴입니다. 영향분석 후 대체 또는 종료하세요. | PF-04M |
| `ROLE_SELF_ESCALATION_DENIED` | 자신의 Role이나 권한을 직접 상향할 수 없습니다. 별도 승인자를 지정하세요. | PF-01U/OP-06C/TN-01U |
...
| `STEP_UP_REQUIRED` | 민감 작업입니다. 재인증(MFA/패스키) 후 진행하세요. | OP-06B |
...
| `FEATURE_NOT_ENABLED` | 이 회사에서 사용하지 않는 기능입니다. (업종·기능 설정 확인) | OP-03/OP-06D |
| `PAYMENT_PAST_DUE` | 미납으로 일부 기능이 제한됩니다. 청구·수납을 확인하세요. | OP-04 |

### 26.5 화면 ID 체계 정리 (BF→OP 매핑·표기 충돌 해소)

**(1) BF → 채널 재배치 매핑**: BF-00→OP-00A, BF-01→OP-02(현재 관리회사 조직·소속; 관리회사 생명주기와 플랫폼 사용자·내부조직은 PF-01), BF-02→OP-03, BF-03→OP-08A, BF-04→OP-06A, BF-05→OP-06B, BF-06→OP-07, BF-07→OP-06C, **BF-08 표준 원본·버전은 PF-04 / 배포운영은 OP-05**, BF-09→TN-01, BF-10→OP-04, BF-11→TN-01, **BF-12 메뉴 원본·MenuVersion은 PF-04M / OP 발행본 조회는 OP-06D / 그룹별 OP 메뉴권한은 OP-06P**.

**(2) 표기 충돌 해소 규칙**: 2뎁스 **메뉴 코드**(OP-xx/TN-xx/CO-xx, 26.1)와 구 화면설계서의 **화면 ID**(TN-05 전표입력, TN-06 결재함, TN-08 증빙함, TN-09 마감 등)는 동명이의이므로, 본 문서에서 TN-xx가 메뉴 코드인지 화면 ID인지 문맥에 명시한다. 신규 산출물은 26.1의 메뉴 코드를 기준으로 하고, 구 화면 ID는 괄호 병기로만 사용한다.

**(3) 정정 사항(v1.1)**: ① 기장 워크벤치 `OP-12` → `OP-08`, ② 구 OP-02 테넌트 상세 기장 설정 탭 → OP-08A 독립 화면, ③ OP-02는 현재 관리회사의 조직·소속만 관리하고 복수 관리회사 등록·상태·이관은 PF-01/PF-03에서 처리, ④ SA-BAS-08~13 ID-화면명은 25장 절 번호 기준으로 확정, ⑤ 시스템관리자 화면 원본에서 사용자그룹 화면에 중복 표기된 `OP-06A` 칩은 `OP-06G`로 정정, ⑥ 화면 원본의 OP-06D 메뉴 CUD/발행은 PF-04M으로 승격하고 OP-06D는 읽기전용으로 정정한다.
...
1. 상단에 화면명·화면 ID·현재 channel/scope·breadcrumb를 표시하고 PF/OP/TN 셸의 컨텍스트를 고정한다.
...
|---|---|---|---|
| PF-01U / OP-06A / TN-01U 사용자관리 | 이름/loginId/소속, 로그인 분류, 상태, MFA; 이름·Login ID·사용자유형·상태 목록 | 기본정보, 소속/배정, 기본그룹/Role·유효기간, 보안/세션(MFA·활성세션·신뢰기기·로그인이력) | 초대, 저장, 정지/종료, MFA 초기화, 세션종료, 이력조회 |
| PF-01G / OP-06G / TN-01G 사용자그룹관리 | 그룹명/코드, 그룹유형, 상태; 선택적 그룹 트리 | 그룹 기본정보와 구성원 목록을 별도 탭/저장단위로 제공 | 그룹 CUD, 구성원 추가/종료, 복사, 변경이력 |
| PF-01P / OP-06P / TN-01P 그룹별 메뉴권한 | 사용자그룹, channel, 메뉴명/코드, 정책 필터 | PUBLISHED 메뉴 목록의 현재정책·상속근거·최종결과, 사용자/시점 미리보기, 변경이력 | INHERIT/ALLOW/DENY 일괄저장, 변경비교, 최종권한 미리보기 |
| PF-04M 메뉴관리 | 메뉴명/코드, channel(PF/OP/TN/CO), 유형(GROUP/MENU/TAB/ACTION), 버전/상태; 트리는 탐색필터 | **메뉴목록 그리드가 주 편집단위**, 선택 행 상세, 버전비교·사용처·미리보기 | 신규/수정/종료, 검증, 영향분석, 내보내기, 검토/승인/발행/정지/롤백 |
| OP-06D 발행 메뉴조회 | 메뉴명/코드, OP channel, 현재 PUBLISHED 버전 | 메뉴목록·Role/그룹 영향·route/feature flag 읽기전용 | 조회, 영향 미리보기, 내보내기; CUD/발행 없음 |

#### 26.6.3 메뉴관리의 메뉴목록 표준

PF-04M의 기본 보기는 트리 자체가 아니라 다음 열 순서의 **메뉴목록 그리드**다. 트리에서 노드를 선택하면 해당 노드와 하위 메뉴로 목록을 필터링하고, 목록 행 선택이 상세 편집 대상을 결정한다.

| 순서 | 열 | 표시·편집 규칙 |
|---:|---|---|
| 1 | 메뉴코드 `code` | 버전 내 channel별 유일, 발행 후 식별자 변경 금지 |
...
| 7 | 화면 ID `screen` | 화면 인벤토리와 연결, ACTION은 대상 action code 연결 가능 |
...
| 검증 | 완료 기준 |
|---|---|
| 범위 격리 | PF/OP/TN 사용자는 자기 scope의 사용자·그룹·정책만 조회·변경하고 메뉴 원본 쓰기는 PF-04M에서만 가능 |
...
| 메뉴 마스터 | `Menu`, `MenuI18n`, `MenuRouteBinding`, `MenuVersion`, `MenuPublishLog`, `RoleMenuPermission`, `MenuActionGrant` | PF-04M 플랫폼 원본·목록·버전·발행; OP/TN은 발행본 사용 |
...
| `MenuCatalogService`/`MenuVersionService` | PF-04M 목록형 메뉴 원본과 검토·승인·발행·롤백·영향분석 |
...
| `MenuCatalogService` | PF-04M 메뉴목록·계층·다국어·route·feature flag 검증 |
...
| `GET·POST` | `/api/platform/menu-versions` · `/api/platform/menu-versions/{id}/menus` | PF-04M 메뉴 버전·목록 관리 |
