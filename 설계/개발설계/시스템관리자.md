...
|---|---|
| 사용자 그룹 | `OperatorUser` |
| 주요 채널 | 운영 콘솔 OP-00~OP-12 |
| 핵심 역할 | 인증정책, 콘솔 IP 통제, 테넌트 인프라, 배치/연계, 로그 관리, 개인정보보호, KMS, DLP, DR |
...
| OP-10 로그 관리 | 시스템 로그, 접근·변경·개인정보 접근 로그, 연계 로그, 보안 이벤트, 감사로그 해시체인, 감사 리포트 | 전체. 민감 로그 상세와 리포트 생성은 Step-up·사유 필요 | §17, §20 |
| OP-11 AI·규칙 운영 | 챗봇 가드레일, QueryTool 허용목록, 모델·룰 보안 정책 검토 | 가드레일 제한 | §11.7, §11.8, §26.3 |
| OP-12 개인정보보호 | 개인정보 카탈로그, 마스킹 정책, 평문 접근 승인, 보존·파기 정책, DLP 정책, 정책 실행 로그 | 전체. 정책설정·평문 조회·파기 실행은 Step-up·사유·승인 필요 | §17, §20, §21 |
...
| 로그 | 기록 대상 |
|---|---|
| `SystemLog` | 서비스 기동/중지, 배치 스케줄러, 워커 상태, 시스템 설정 변경, 장애 타임라인 |
| `ErrorLog` | API/배치/연계 오류, 예외 스택 요약, 사용자 노출 오류코드, traceId |
| `LoginHistory` | 로그인/로그아웃/실패, 계정잠금, MFA 성공/실패, 신규 기기·지역 |
...
| `IntegrationLog` | 외부 연계 호출·응답 요약, 원천키, 멱등키, 재전송 이력 |
| `AuditLogChain` | 중요 로그 해시체인, 위변조 검증 |
| `SecurityEvent` | 권한 외 접근, 대량 다운로드, 비정상 시간대 접근, 반복 실패 |
| `DataDestructionLog` | 개인정보·계약종료 데이터 파기 대상, 방법, 수행자, 승인자, 시각 |

**기능**:

- OP-10 「로그 통합조회」 탭에서 로그유형, 기간, 사용자, 테넌트, traceId, 심각도, 무결성 상태로 조회한다.
- 로그 목록에서 행을 선택하면 같은 화면의 세부정보 영역에 로그ID, 발생시각, 행위자, 권한, 테넌트, 대상, 행위, 접속IP, 세션ID, 승인번호, 발생 화면/API, 보존정책, 원문 요약, 변경 전후값을 표시한다.
- 로그 검색은 민감정보 기본 마스킹을 적용하고, 민감 로그 상세는 Step-up + 사유 입력 후 조회한다.
- 로그 원본은 append-only로 저장하며 화면에서는 조회·리포트·무결성 검증만 허용한다. 원본 삭제·수정 버튼은 제공하지 않는다.
- `AuditLogChain` 해시체인 검증 실패 시 `SecurityEvent` 및 보안 인시던트를 자동 생성하고 감사자에게 알림을 발송한다.
- 대량 다운로드·복호화·평문 조회 로그는 임계 초과 시 DLP 이벤트로 승격한다.
- 감사 리포트는 PDF/Excel로 생성하되, 출력물에도 화면과 동일한 마스킹·워터마크 정책을 적용한다.

**OP-10 화면 탭**:

| 탭 | 관리 대상 | CRUD 여부 | 주요 통제 |
|---|---|---|---|
| 로그 통합조회 | SystemLog, ErrorLog, LoginHistory, OperatorActivityLog, AdminAccessLog, PersonalDataAccessLog, DataChangeLog, IntegrationLog, SecurityEvent, AuditLogChain, DataDestructionLog | 원본 CRUD 불가. 조회·목록 선택 상세·무결성검증·리포트 생성 | append-only, Step-up, 기본 마스킹, traceId 추적 |
| 무결성·해시체인 | `AuditLogChain`, 중요 로그 hash, WORM checksum | 원본 CRUD 불가. 검증 실행·결과 저장 | sequence, prevHash, payloadHash, writer 검증 |
| 감사 리포트 | `AuditReport`, 리포트 생성 파라미터, 다운로드 이력 | 원본 CRUD 불가. 리포트 생성·다운로드 | Step-up, 사유, 워터마크, 보존정책 |

### 4.8 개인정보보호 정책설정·실행로그(OP-12)

**목적**: 개인정보 카탈로그, 마스킹, 평문 접근 승인, 보존·파기, DLP 정책을 별도 메뉴에서 설정하고 실제 실행 로그를 정책 단위로 추적한다.

**OP-12 화면 탭**:

| 탭 | 관리 대상 | CRUD 여부 | 주요 통제 |
|---|---|---|---|
| 정책설정 | `PersonalDataCatalog`, `MaskingPolicy`, `RetentionPolicy`, `DlpRule`, 평문 접근 승인 정책 | 조회·추가·수정·삭제(폐기)·저장, 영향 분석 | 처리목적, 법적근거, 보유기간, 암호화/마스킹, 승인자 분리 |
| 실행 로그 | `PersonalDataAccessLog`, `MaskingApplyLog`, `PlainTextAccessRequest`, `DataDestructionLog`, `DlpEventLog` | 원본 CRUD 불가. 조회·상세·리포트 생성 | policyId, traceId, actor, target, purpose, approvalId, result 필수 |

- 실행 로그 목록에서 행을 선택하면 같은 화면의 세부정보 영역에 실행로그ID, 정책ID, 실행유형, 발생시각, 요청자, 테넌트, 대상, traceId, 승인번호, 승인상태, 만료, 보호처리, 처리목적, 실행내용을 표시한다.
- 실행 로그의 세부항목은 로그 종류별 실행 대상 데이터로 구성한다. 예를 들어 `PersonalDataAccessLog`는 개인정보를 접근한 화면명, 대상자명, 조회자명, 조회 개인정보 항목, 조회 사유, 승인경로를 표시하고, `MaskingApplyLog`는 출력화면, 파일명, 다운로드 요청자, 마스킹 대상 필드, 대상자 범위, 다운로드 링크 만료를 표시한다.
- 실제 실행 대상자를 확인할 때는 실행 로그 행을 선택한 뒤 대상자 세부내역 영역에서 대상자명/ID로 조회한다. 대상자 세부내역은 대상자명, 대상자ID, 소속/구분, 개인정보 항목, 실행값, 실행결과를 표시하고, 기본값은 마스킹된 값이다.
- 실행 로그 세부내용은 원본 수정·삭제 없이 조회 전용으로 제공하며, 평문값은 승인 완료 전 기본 마스킹 상태로 표시한다.

### 4.8.1 개인정보 카탈로그 관리

**목적**: 개인정보가 저장되는 테이블·컬럼과 처리 목적을 사전 식별하여 암호화·마스킹·보유기간·접근통제를 일관되게 적용한다.

| 항목 | 설명 |
|---|---|
| 개인정보 분류 | 일반 개인정보, 고유식별정보, 금융정보, 인사/급여, 인증/보안 정보 |
| 데이터 위치 | schema/table/column, API field, 파일/첨부 경로, 백업 포함 여부 |
| 처리 목적 | 원천세·4대보험 신고, 자금 이체, 고객 연락, 계약 이행, 감사 대응 등 |
| 법적 근거 | 법정 신고, 계약, 동의, 정당한 이익, 보존 의무 |
| 보유기간 | 항목별 보유기간과 기산점. 계약 종료·퇴사·사업연도 종료 등 |
| 보호 조치 | 암호화 여부, 마스킹 여부, 평문 조회 가능 Role, 다운로드 가능 여부 |
| 담당자 | 정책 소유자, 승인자, 변경 검토자 |

**검증**:

- 고유식별정보·금융정보·급여정보가 암호화 또는 마스킹 없이 저장되면 저장을 차단한다.
- 보유기간, 처리목적, 법적근거가 없는 카탈로그 항목은 발행할 수 없다.
- 카탈로그 변경 전 영향 화면, API, Excel 다운로드, 배치, 로그 필드를 분석해야 한다.
- 카탈로그에서 폐기된 항목은 신규 수집·평문 조회·다운로드 정책에 사용할 수 없다.

**엔티티**: `PersonalDataCatalog`, `PersonalDataFieldUsage`, `PersonalDataCatalogHistory`.

### 4.8.2 마스킹 정책·평문 접근 승인

**목적**: 화면·목록·보고서·엑셀·로그·녹화에서 개인정보를 기본 마스킹하고, 평문 조회는 제한된 승인 절차로만 허용한다.

| 기능 | 상세 |
|---|---|
| 마스킹 정책 CRUD | 항목, 화면, Role, 출력 채널별 마스킹 규칙을 추가·수정·폐기·저장 |
| 평문 조회 승인 | 요청자, 대상 항목, 목적, 건수, 만료시간, 승인자, 사유 저장 |
| 자동 재마스킹 | 승인 만료 또는 화면 이탈 시 평문값 자동 재마스킹 |
| 다운로드 통제 | 평문 다운로드 원칙 금지. 예외 시 2인 승인, 워터마크, 링크 만료, DLP 이벤트 |
| 접근로그 | 평문 조회·복호화·다운로드는 `PersonalDataAccessLog`에 사용자/시각/대상/목적/건수 기록 |

**검증**:

- 평문 조회 요청에는 목적과 대상 항목, 건수, 만료시간이 필수다.
- 요청자와 승인자가 동일하면 승인할 수 없다.
- 민감정보 권한이 없는 Role에는 평문 조회 정책을 부여할 수 없다.
- 다운로드 파일은 마스킹 정책과 워터마크 정책을 동시에 통과해야 한다.

**엔티티**: `MaskingPolicy`, `PlainTextAccessRequest`, `PersonalDataAccessLog`, `DlpApproval`.

### 4.8.3 보존기간·파기 정책 관리

**목적**: 개인정보·회계장부·증빙·로그별 보존기간을 관리하고, 보유기간 경과 또는 계약 종료 시 분리보관·파기·소산을 통제한다.

| 기능 | 상세 |
|---|---|
| 보존정책 CRUD | 대상 데이터, 보유기간, 기산점, 경과 후 조치, 법적근거, 보존예외 관리 |
| 파기 대상 산출 | 개인정보 보유기간/계약 종료/임시파일 만료/백업 보존기간 경과 대상 산출 |
| 파기 승인 | 2인 승인, Step-up, 파기 범위·방법·대상 건수 확인 |
| 파기 실행 | 물리 삭제, 논리 삭제, 아카이브 소산, crypto-shredding 중 정책에 따른 실행 |
| 파기 증명 | 수행자, 승인자, 시각, 방법, 대상 hash, 결과를 증명서로 발행 |

**검증**:

- 법정 보존기간이 남은 회계장부·증빙은 파기할 수 없다.
- 계약 종료 전 개인정보 파기는 고객/계약 상태와 보존 예외를 확인해야 한다.
- 백업·아카이브·검색 인덱스·로그 내 개인정보까지 파기 또는 재마스킹 범위에 포함해야 한다.
- 파기 사실 자체는 `DataDestructionLog`로 보존하며, 파기 로그는 삭제할 수 없다.

**엔티티**: `RetentionPolicy`, `ArchiveRecord`, `DataDestructionJob`, `DataDestructionLog`, `DataDestructionCertificate`.
...
| `GET` | `/api/operator/logs` | 시스템·접근·변경·개인정보·연계·보안 로그 통합조회 | 민감 로그 Step-up, 기본 마스킹 |
| `POST` | `/api/operator/logs/audit-chain/verify` | 감사로그 해시체인 검증 | 감사자/SEC_ADMIN |
| `GET·POST·PUT` | `/api/operator/privacy/catalog` | 개인정보 카탈로그 조회·작성·수정 | 필수값·영향 분석 |
| `POST` | `/api/operator/privacy/catalog/{id}/retire` | 개인정보 카탈로그 항목 폐기 | 사용처 영향 분석 |
| `GET·POST·PUT` | `/api/operator/privacy/masking-policies` | 마스킹 정책 조회·작성·수정 | Step-up |
| `POST` | `/api/operator/privacy/plain-access-requests` | 평문 조회·복호화 승인 요청 | 사유, 대상, 건수, 만료시간 |
| `POST` | `/api/operator/privacy/plain-access-requests/{id}/approve` | 평문 접근 승인/거절 | 승인자 분리 |
| `GET` | `/api/operator/privacy/execution-logs` | 개인정보 정책적용·평문조회·다운로드·파기·DLP 실행 로그 조회 | policyId, traceId, Step-up |
| `GET` | `/api/operator/privacy/execution-logs/{id}/subjects` | 실행 로그별 대상자 세부내역 조회 | 대상자명/ID 검색, 마스킹 기본 |
| `GET·POST·PUT` | `/api/operator/retention/policies` | 보존기간 정책 조회·작성·수정 | 법정근거 필수 |
| `POST` | `/api/operator/destruction/jobs` | 개인정보·데이터 파기 작업 생성 | 2인 승인 |
| `POST` | `/api/operator/destruction/jobs/{id}/approve` | 파기 작업 승인 | Step-up |
| `GET` | `/api/operator/destruction/certificates/{id}` | 파기 증명서 조회 | 감사자/SEC_ADMIN |
...
| `LOG_IMMUTABLE` | 로그 원본 수정·삭제 시도 | 요청 차단, 보안 이벤트 |
| `LOG_QUERY_STEP_UP_REQUIRED` | 민감 로그 상세 조회에 Step-up 없음 | Step-up 모달 |
| `AUDIT_CHAIN_INVALID` | 감사로그 해시체인 검증 실패 | 보안 인시던트 생성 |
| `PERSONAL_DATA_CATALOG_REQUIRED` | 민감 필드가 카탈로그에 없음 | 배포·저장 차단 |
| `PERSONAL_DATA_CATALOG_INCOMPLETE` | 목적·법적근거·보유기간·보호조치 누락 | 저장/발행 차단 |
| `MASKING_POLICY_REQUIRED` | 개인정보 항목에 마스킹 정책 없음 | 화면/다운로드 노출 차단 |
| `PLAIN_TEXT_ACCESS_APPROVAL_REQUIRED` | 평문 조회·복호화 승인 없음 | 조회 차단 |
| `PLAIN_TEXT_ACCESS_EXPIRED` | 승인 만료 후 평문 접근 | 자동 재마스킹, 로그 기록 |
| `RETENTION_POLICY_REQUIRED` | 보존기간 정책 없음 | 파기·아카이브 처리 차단 |
| `DATA_DESTRUCTION_APPROVAL_REQUIRED` | 파기 작업 승인 부족 | 파기 실행 차단 |
| `DATA_DESTRUCTION_LEGAL_HOLD` | 법정 보존·소송보존 대상 파기 시도 | 파기 차단 |
...
- OP-10 로그 통합조회는 로그 원본을 수정·삭제하지 않고, 민감 로그 상세 조회 시 Step-up·사유를 요구해야 한다.
- OP-12 개인정보보호 실행 로그는 정책ID, traceId, actor, target, purpose, approvalId, result를 포함하고 원본 수정·삭제가 불가해야 한다.
- 모든 중요 로그는 `traceId`, actor, target, purpose/reason, source, IP/device, result, hash 값을 포함해야 한다.
- `AuditLogChain` 검증 실패는 배포 전·운영 중 모두 보안 인시던트로 승격되어야 한다.
- 개인정보가 저장되는 신규 테이블·컬럼·파일 필드는 `PersonalDataCatalog` 등록 없이는 배포할 수 없다.
- 개인정보 카탈로그 항목은 처리목적, 법적근거, 보유기간, 암호화/마스킹 여부, 담당자가 누락되면 발행할 수 없다.
- 마스킹 정책 없는 개인정보 필드는 화면·목록·보고서·Excel 다운로드에 노출할 수 없다.
- 평문 조회·복호화·평문 다운로드는 `PlainTextAccessRequest` 승인과 `PersonalDataAccessLog` 기록 없이는 불가하다.
- 보존기간 정책 없는 개인정보·로그·증빙 데이터는 아카이브·파기 처리할 수 없다.
- 파기 작업은 법정 보존기간·소송보존·계약상태·백업/아카이브 포함 여부를 검증하고, 승인 후 `DataDestructionLog`와 증명서를 남겨야 한다.
...
| 민감 로그 상세 조회 | 보안관리자, 감사자 | 인앱 | 조회자, 로그유형, 대상, 사유, traceId |
| 감사로그 해시체인 실패 | 보안관리자, 감사자 | 긴급 알림 | 실패 구간, root hash, 영향 로그, 인시던트 번호 |
| 개인정보 카탈로그 변경 | 시스템관리자, 개인정보 담당, 감사자 | 인앱 | 항목, 위치, 처리목적, 보유기간, 영향 화면/API |
| 마스킹 정책 변경 | 시스템관리자, 개인정보 담당 | 인앱 | 대상 필드, 화면/출력 채널, Role, 변경 전후값 |
| 평문 접근 승인 요청 | 승인자, 보안관리자 | 인앱 | 요청자, 대상 항목, 건수, 목적, 만료시간 |
| 개인정보 보유기간 경과 | 시스템관리자, 개인정보 담당 | 인앱/이메일 | 대상 테넌트, 항목, 건수, 처리기한 |
| 파기 작업 승인 대기 | 승인자, 감사자 | 인앱 | 파기대상, 건수, 방법, 법적근거, 요청자 |
| 파기 완료/실패 | 시스템관리자, 감사자 | 인앱/리포트 | 작업ID, 결과, 증명서, 실패 건수 |
...
- 시스템관리자는 OP-09/OP-10/OP-12에서 배치·로그·개인정보보호 현황을 조회하고 조치할 수 있다.
...
- 시스템관리자는 OP-10에서 시스템 로그, 오류 로그, 로그인 이력, 운영자 활동 로그, 접근 로그, 개인정보 접근 로그, 데이터 변경 로그, 연계 로그, 보안 이벤트, 감사로그 해시체인, 파기 로그를 통합 조회할 수 있다.
- 로그 원본은 화면에서 수정·삭제할 수 없고, 상세 조회·내보내기는 권한·Step-up·사유·마스킹 정책을 통과해야 한다.
- 시스템관리자는 OP-12에서 개인정보 카탈로그를 추가·수정·폐기·저장하고, 처리목적·법적근거·보유기간·암호화·마스킹 여부를 관리할 수 있다.
- 시스템관리자는 OP-12에서 화면/Role/출력채널별 마스킹 정책과 평문 접근 승인 정책을 추가·수정·삭제·저장할 수 있다.
- 개인정보 평문 조회·복호화·다운로드는 승인 요청, 승인자 분리, 제한시간, 자동 재마스킹, `PersonalDataAccessLog` 기록을 만족해야 한다.
- 시스템관리자는 OP-12에서 보존기간 정책을 관리하고, 파기 대상 산출·승인·실행·파기증명서 조회를 수행할 수 있다.
- 시스템관리자는 OP-12에서 정책적용, 평문조회, 다운로드, 파기, DLP 실행 로그를 정책ID 기준으로 조회할 수 있다.
- 감사로그 해시체인 검증 실패 시 보안 인시던트가 자동 생성되고 원본 로그는 격리된다.
...
- 개인정보 카탈로그에 등록되지 않은 민감 필드는 운영 화면, API 응답, 다운로드, 로그 상세에 노출될 수 없다.
- 개인정보 마스킹 정책은 화면 조회, 로그 조회, 녹화, 보고서, Excel 다운로드에 동일하게 적용된다.
- 평문 접근 승인 만료 후에는 자동 재마스킹되어야 하며, 승인 만료 후 접근은 차단된다.
- 파기 작업은 승인·법정 보존 검증·파기이력 없이는 완료될 수 없다.
...
| 테스트 | 시나리오 |
|---|---|
| 권한 테스트 | `SEC_ADMIN`이 OP-10/OP-12는 전체 접근, OP-08 기장 워크벤치는 미노출 |
...
| 로그 통합조회 테스트 | SystemLog/ErrorLog/LoginHistory/AdminAccessLog/PersonalDataAccessLog/DataChangeLog/IntegrationLog/SecurityEvent가 기간·사용자·traceId 조건으로 조회됨 |
| 로그 원본 불변 테스트 | 로그 원본 수정·삭제 API 호출 시 `LOG_IMMUTABLE` |
| 민감 로그 Step-up 테스트 | Step-up 없이 PersonalDataAccessLog 상세 조회 시 `LOG_QUERY_STEP_UP_REQUIRED` |
| 감사로그 해시체인 테스트 | 임의 변조된 로그 구간 검증 시 `AUDIT_CHAIN_INVALID` 및 SecurityEvent 생성 |
| 개인정보 카탈로그 필수 테스트 | 민감 필드가 `PersonalDataCatalog`에 없으면 `PERSONAL_DATA_CATALOG_REQUIRED` |
| 개인정보 카탈로그 누락 테스트 | 처리목적·법적근거·보유기간 누락 시 `PERSONAL_DATA_CATALOG_INCOMPLETE` |
| 마스킹 정책 필수 테스트 | 개인정보 필드에 MaskingPolicy가 없으면 화면·다운로드 노출 차단 |
| 평문 접근 승인 테스트 | 승인 없는 평문 조회 시 `PLAIN_TEXT_ACCESS_APPROVAL_REQUIRED` |
| 평문 접근 만료 테스트 | 승인 만료 후 같은 화면에서 자동 재마스킹되고 `PLAIN_TEXT_ACCESS_EXPIRED` 기록 |
| 파기 법정보존 테스트 | 보존기간 남은 데이터 파기 시 `DATA_DESTRUCTION_LEGAL_HOLD` |
| 파기 승인 테스트 | 2인 승인 없는 파기 실행 시 `DATA_DESTRUCTION_APPROVAL_REQUIRED` |
